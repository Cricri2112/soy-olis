...
- `src/pages/admin/`: vistas del panel (Login, Productos, EditarProducto para `/admin/nuevo` y `/admin/:id`). `admin.css` tiene los estilos compartidos del panel: campos y botones de 52 px de alto, letra de 16 px para que iOS no haga zoom.
...
- `src/components/admin/`: piezas del panel (`LayoutAdmin` con la barra superior y el botón Salir, `FilaProducto` con el interruptor visible/oculto, `FormularioProducto` con lo obligatorio arriba y lo opcional plegado, `SelectorTallesAdmin` con chips y campo "otro", `FotosAdmin` para agregar, ordenar y quitar fotos, `EliminarProducto` con confirmación en el lugar).
- Eliminar producto: primero se borran los archivos del bucket y después la fila; talles y fotos se van solos por el `on delete cascade` de la base.
- Slug: se genera solo desde el nombre (`utils/slug.js`) y, antes de guardar, `slugDisponible` en `productosApi.js` le agrega `-2`, `-3`... si ya existe. Los talles se guardan borrando los anteriores e insertando la lista nueva.
- Nombre repetido: el formulario avisa (no bloquea) si ya hay otro producto con el mismo nombre; un segundo "Publicar" guarda igual. No avisa si el nombre no cambió.
- Fotos del panel: nada se sube al elegirlas. Se comprimen en el navegador (`utils/imagen.js`: 1600 px de lado máximo, JPEG 0.85) y quedan en memoria; al guardar el formulario, `sincronizarFotos` en `fotosApi.js` borra las quitadas (fila y archivo del bucket), sube las nuevas a `{producto_id}/{timestamp}-{nombre}.jpg` y guarda el orden final. La primera de la lista es la principal.
...
- `src/data/`: acceso a datos, sin React. `productosApi.js` consulta Supabase y devuelve productos "normalizados" (talles ordenados, fotos como `{ id, ruta, url }`). `fotosApi.js` maneja el bucket y la tabla `fotos`. `authApi.js` envuelve Supabase Auth. `contacto.js` tiene los textos fijos de la tienda.
...
- `src/utils/`: funciones puras y chicas (`precio.js`, `whatsapp.js`, `catalogo.js`, `slug.js`, `imagen.js`).
- `public/`: solo `logo.svg` (logo de la marca), `hero.webp` (portada de la Home, 1600 px) y `favicon.svg`. Las fotos de las prendas viven en el bucket de Supabase, no en el repo.
...
- Las fotos de las prendas se suben desde el panel y viven en el bucket `productos` de Supabase, en `{producto_id}/{timestamp}-{nombre}.jpg`. Nunca se suben fotos de prendas al repo.
- Se comprimen en el navegador antes de subir (`utils/imagen.js`). Si cambia el tamaño máximo o la calidad, es ahí.
...
- La galería del detalle se desliza con el dedo; con mouse hay flechas y los indicadores de abajo son botones.
...
- Sin dependencias nuevas en el sitio sin justificarlo antes. Las herramientas de desarrollo van en `devDependencies`.
...
3. Verificar que las tres vistas públicas cargan y la navegación entre ellas funciona.
4. Si se tocó el panel: login, lista, alta o edición según corresponda, contra la base real.
5. Verificar el layout en viewport móvil.

Si algo falla, arreglarlo antes de reportar. **Nunca decir que algo está listo sin haberlo verificado.**

## Tests

- Por ahora **no** hay suite de tests. La verificación es manual, siguiendo la sección de arriba, y contra la base real.
- Los tests formales se incorporan en la Etapa 2 (stock, ventas), cuando exista lógica de negocio que proteger. Buenos primeros candidatos: `utils/slug.js`, `utils/catalogo.js` y `utils/whatsapp.js`, que son funciones puras.
