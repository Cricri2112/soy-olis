...
- `src/components/admin/`: piezas del panel (`LayoutAdmin` con la barra superior y el botón Salir, `FilaProducto` con el interruptor visible/oculto, `FormularioProducto` con lo obligatorio arriba y lo opcional plegado, `SelectorTallesAdmin` con chips y campo "otro", `FotosAdmin` para agregar, ordenar y quitar fotos).
